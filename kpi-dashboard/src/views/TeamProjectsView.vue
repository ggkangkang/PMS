...
import {
  projects, users, getDepartmentStaff, getMonthlyEval, getFinalMonthlyScore,
...
// My direct reports
const myDirectReports = computed(() => {
  return new Set(getSubordinates(currentUser.value.id).map(s => s.id))
})
...
const filteredByProject = computed(() => {
  if (projectFilter.value === 'all') return staff.value
  return staff.value.filter(s => s.projectId === projectFilter.value)
})

const filteredByRole = computed(() => {
  if (roleFilter.value === 'all') return filteredByProject.value
  return filteredByProject.value.filter(s => s.role === roleFilter.value)
})

const filteredByTeam = computed(() => {
  if (!myTeamOnly.value) return filteredByRole.value
  return filteredByRole.value.filter(s => myDirectReports.value.has(s.id))
})

const filters = computed(() => {
  const base = filteredByTeam.value
...
const filteredStaff = computed(() => {
  ...
  return [...list].sort((a, b) => {
    ...
    switch (sortBy.value) {
      ...
      case 'project':
        vA = getProjectById(a.projectId)?.name || ''
        vB = getProjectById(b.projectId)?.name || ''
...
function openEmployee(emp) {
  ...
  // Check if this user is a direct report (then we show full rating form)
  // Otherwise show read-only behaviour from their actual supervisor
  const isMyReport = myDirectReports.value.has(emp.id)

  if (isMyReport && ev?.supervisor) {
    // Already rated by me — load existing
    ...
    isSaved.value = true
  } else if (isMyReport && ev) {
    // My report, not rated yet — init empty form
  ...
  } else if (!isMyReport && ev?.supervisor) {
    // Not my report but supervisor already rated — show read-only
    supervisorForm.commitmentRatings = { ...ev.supervisor.commitmentRatings }
    supervisorForm.behaviourRatings = { ...(ev.supervisor.behaviourRatings || {}) }
    supervisorForm.notes = ev.supervisor.notes || ''
    isSaved.value = true
  } else if (ev) {
    // Not my report, not yet rated — show empty read-only
    const kpis = getKPIsForRole(emp.role)
    const empty = {}; kpis.forEach(k => empty[k.id] = 0)
    supervisorForm.commitmentRatings = empty
    const bEmpty = {}; behaviourPillars.forEach(p => bEmpty[p.id] = 0)
    supervisorForm.behaviourRatings = bEmpty
    supervisorForm.notes = ''
    isSaved.value = true // read-only for non-reports
  } else {
...
function isDirectReport(emp) {
  return myDirectReports.value.has(emp.id)
}
...
<template>
  <div class="max-w-7xl mx-auto animate-fade-in">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-lg font-bold text-txt-heading">Team & Projects</h1>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
      ...
      <div class="sm:ml-auto flex items-center gap-3 flex-wrap">
        <!-- Project filter -->
        <select v-model="projectFilter" class="select-field text-sm py-1.5 w-44">
        ...
        <label class="flex items-center gap-2 cursor-pointer select-none group">
          <div class="relative">
            <input type="checkbox" v-model="myTeamOnly" class="sr-only peer" />
            <div class="w-9 h-5 bg-gray-200 rounded-full peer-checked:bg-brand-primary transition-colors"></div>
          ...
          <span class="text-sm text-txt-subtitle group-hover:text-txt-body transition-colors whitespace-nowrap">My Team</span>
        </label>
      </div>
    </div>
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-line text-left">
            ...
            <th class="px-5 py-2.5 text-[11px] font-semibold text-txt-disabled uppercase tracking-wider cursor-pointer select-none hidden md:table-cell" @click="toggleSort('project')">Project {{ sortIcon('project') }}</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-5 py-3">
              <div class="flex items-center gap-3">
                <div class="relative shrink-0">
                  <div :class="[s.avatarColor || 'bg-brand-primary', 'w-8 h-8 rounded-full text-white text-[10px] font-bold flex items-center justify-center']">{{ s.initials }}</div>
                  <div v-if="isDirectReport(s)" class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-brand-primary text-white rounded-full flex items-center justify-center text-[7px] font-bold ring-1.5 ring-white" title="Direct report">★</div>
                </div>
                <div>
            ...
            <td class="px-5 py-3 text-sm text-txt-body hidden md:table-cell">
              <span class="tag bg-surface-gray text-txt-subtitle text-[10px]">{{ getProjectById(s.projectId)?.name || '—' }}</span>
            </td>
      ...
      <div v-if="showDrawer && selectedEmployee" class="fixed inset-0 z-50 flex items-stretch justify-end">
        ...
        <div class="relative z-10 w-full max-w-2xl bg-surface-white shadow-xl overflow-y-auto animate-slide-right">
          <div class="p-6 space-y-5">
            ...
            <!-- Direct report badge -->
            <div v-if="isDirectReport(selectedEmployee)" class="flex items-center gap-2 px-3 py-2 bg-brand-primary/5 border border-brand-primary/20 rounded-container">
              <span class="text-brand-primary text-sm">★</span>
              <span class="text-xs text-brand-primary font-medium">Direct Report — You can rate this team member</span>
            </div>
            <div v-else class="flex items-center gap-2 px-3 py-2 bg-surface-gray border border-line rounded-container">
              <span class="text-txt-muted text-sm">👁</span>
              <span class="text-xs text-txt-muted font-medium">View Only — Supervised by {{ selectedEmployee.reportsTo }}</span>
            </div>
            ...
            <div v-if="getMonthlyEval(selectedEmployee.id, selectedMonth)">
              <h4 class="text-[11px] font-semibold text-txt-muted uppercase tracking-wider mb-3">Self KPI Ratings (50%)</h4>
              <div class="space-y-2">
              ...
              <div class="space-y-3">
                <div v-for="pillar in behaviourPillars" :key="pillar.id" class="p-4 rounded-container border border-line">
                  ...
                  <div class="flex gap-1.5 mb-2">
                    <button v-for="r in behaviourRatingScale" :key="r.value" @click="supervisorForm.behaviourRatings[pillar.id] = r.value" :disabled="isSaved || !isDirectReport(selectedEmployee)"
            ...
            <!-- Submit (only for direct reports) -->
            <div class="flex gap-2 mt-5">
              <button v-if="isDirectReport(selectedEmployee) && !isSaved && getMonthlyEval(selectedEmployee.id, selectedMonth)" @click="saveBehaviour" class="btn-primary flex-1"
              ...
              >Submit & Lock</button>
              <div v-else-if="isDirectReport(selectedEmployee) && isSaved && getMonthlyEval(selectedEmployee.id, selectedMonth)" class="flex-1 text-center py-2.5 bg-surface-success rounded-btn border border-green-200"><p class="text-sm font-bold text-txt-success">✓ Behaviour Rated & Locked</p></div>
              <div v-else-if="!isDirectReport(selectedEmployee)" class="flex-1 text-center py-2.5 bg-surface-gray rounded-btn border border-line"><p class="text-sm text-txt-muted">View-only mode</p></div>
